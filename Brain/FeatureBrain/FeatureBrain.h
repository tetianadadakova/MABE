...
public:
	...
	int c = 0;
	int g = 0;
	FeatureGate(std::vector<int> &inAddr, std::vector<int> &outAddr, std::vector<std::vector<double>> &theTable, int xx, int yy);
    void update(std::vector<double> &nodes, std::vector<double> &nextNodes);
};


class FeatureBrain : public AbstractBrain {
    std::vector<std::shared_ptr<FeatureGate>> Gates;
...
public:
	...
	static std::shared_ptr<ParameterLink<bool>> matchColumnIOPL;
	static std::shared_ptr<ParameterLink<double>> adjacentColumnReachPL;
	int nrOfHiddenStates, nrOfColumns, widthOfColumns;
	bool matchColumnIO;
    
    FeatureBrain() = default;
    FeatureBrain(int _nrInNodes, int _nrOutNodes, std::shared_ptr<ParametersTable> PT_);
...
inline std::shared_ptr<AbstractBrain> FeatureBrain_brainFactory(int ins, int outs, std::shared_ptr<ParametersTable> PT = Parameters::root) {
    return std::make_shared<FeatureBrain>(ins, outs, PT);
}
